...
def rnd_walk_workers(graph, deep_walk_arguments, permuted_idx, proc_begin, proc_end, return_dict):
...
class DeepWalk_Original(object):
    ...
    def generate_walks(self, deep_walk_arguments, graph, workers):
        ...
        for i in range(workers):
            ...
            if i == workers - 1:
                proc_end = len(graph)
            p = mp.Process(target=rnd_walk_workers, args=(graph, deep_walk_arguments, permuted_idx, proc_begin, proc_end, return_dict))
            jobs.append(p)
        for p in jobs:
            p.start()
        for proc in jobs:
